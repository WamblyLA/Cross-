import { resolveWsUrl } from "../../lib/config/env";
import { peekSessionToken } from "../session/sessionStorage";
import type {
...
function createAuthorizedWebSocket(url: string, token: string) {
...
class CloudRealtimeClient {
  ...
  private openSocket() {
    ...
    this.setStatus("connecting");
    this.socket = createAuthorizedWebSocket(resolveWsUrl("/ws"), token);
  ...
  private sendMessage(message: unknown) {
    if (!this.socket || this.socket.readyState !== WebSocket.OPEN) {
      throw new Error("WebSocket пока не подключён.");
    }
  ...
  private rejectAllPendingAcks(message: string) {
    this.pendingAcks.forEach((waiters) => {
